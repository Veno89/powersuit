...
def main() -> None:
    ...
    required = [root / relative for relative in (*AIM_FILES, *RIFLE_FILES)]
    missing = [str(path.relative_to(root)) for path in required if not path.exists() or path.stat().st_size < 4096]
    if missing:
