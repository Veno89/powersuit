...
def _validate_animation_invariants(armature: bpy.types.Object) -> dict[str, object]:
    if int(armature.get("ps_v2_rig_upgrade_version", 0)) < 1:
        raise RuntimeError(
            "Validation requires the controlled V2 arm-proportion upgrade."
        )
...
def _hand_helper_metrics(armature: bpy.types.Object) -> dict[str, float]:
    ...
    return {
        "right_hand_to_grip_target_m": (right_hand - right_target).length,
        "left_hand_to_foregrip_target_m": (left_hand - left_target).length,
    }
...
def _aim_pose_geometry_metrics(
    ...
    delta = sight_world - visor_center
    ...
    if stock_to_anchor > 0.075:
        failures.append(f"stock/stance anchor offset={stock_to_anchor:.3f} m")
    ...
    if metrics["rifle_generator_version"] < 102:
...
def _render_one(
    ...
    if view == "front_3q":
        ...
        grip_point = grip.matrix_world.translation if grip is not None else helper
        lateral_sign = 1.0 if (helper - root.matrix_world.translation).dot(rifle_right) >= 0.0 else -1.0
        camera_side = rifle_right * lateral_sign
        target = (wrist + helper + grip_point) / 3.0 - rifle_up * 0.045
        # Outside/rear/below the pistol grip.  This deliberately clears the
        # receiver and forearm armour so palm, thumb side and grip remain visible.
        location = (
            target
            + camera_side * 0.96
            - rifle_forward * 0.18
        ...
        lateral_sign = 1.0 if (helper - root.matrix_world.translation).dot(rifle_right) >= 0.0 else -1.0
        camera_side = rifle_right * lateral_sign
        target = (wrist + helper + grip_point) / 3.0 - rifle_up * 0.035
        # Slightly below/outside so the now-connected finger cluster can be judged
        # against the compact foregrip instead of disappearing behind the rail.
        location = (
            target
            + camera_side * 0.90
            + rifle_forward * 0.10
        ...
        rifle_up = (rifle_basis @ Vector((0.0, 0.0, 1.0))).normalized()
        scope = require_weapon_helper(root, ROLE_SIGHT_OCULAR).matrix_world.translation
        stock = require_weapon_helper(root, ROLE_STOCK_CONTACT).matrix_world.translation.copy()
        shoulder = Vector(root.get("ps_aim_shoulder_anchor_world", tuple(stock)))
        visor = bpy.data.objects.get("Helmet_Visor")
        visor_point = visor.matrix_world.translation if visor is not None else bone_head_world(armature, "Head")
        lateral_sign = 1.0 if (scope - visor_point).dot(rifle_right) >= 0.0 else -1.0
        camera_side = rifle_right * lateral_sign
        target = visor_point.lerp(scope, 0.52).lerp((stock + shoulder) * 0.5, 0.30)
        # Outside/rear view that keeps the helmet side plate from sitting directly
        # between camera and ocular.  Stock/shoulder remain visible at frame edge.
        location = (
            target
            - rifle_forward * 0.52
            + camera_side * 0.88
...
def main() -> None:
    ...
    try:
        ...
        if hand_metrics["right_hand_to_grip_target_m"] > 0.015:
            automated_blockers.append(
                f"right hand/grip target={hand_metrics['right_hand_to_grip_target_m']:.3f} m"
            )
        if hand_metrics["left_hand_to_foregrip_target_m"] > 0.020:
            automated_blockers.append(
                f"left hand/foregrip target={hand_metrics['left_hand_to_foregrip_target_m']:.3f} m"
            )
