...
"%BLENDER_INPUT%" --background --factory-startup --python-exit-code 1 --python-expr "import bpy; assert bpy.app.version[:2] >= (5, 2), 'Blender 5.2 or newer is required'" >nul 2>&1
if errorlevel 1 (
  echo.
...
for %%I in ("%BLENDER_INPUT%") do > "blender_path.txt" echo "%%~fI"
