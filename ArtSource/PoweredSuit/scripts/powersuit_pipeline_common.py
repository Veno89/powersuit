...
def require_character_asset_versions(armature: bpy.types.Object) -> None:
    """Reject stale rig/shell inputs before pose solving, validation, or export."""
    rig_version = int(armature.get("ps_v2_rig_upgrade_version", 0))
    if rig_version < REQUIRED_RIG_UPGRADE_VERSION:
        raise RuntimeError(
            "The controlled V2 rig upgrade is stale or missing "
            f"(found {rig_version}, require {REQUIRED_RIG_UPGRADE_VERSION})."
        )
    for name in ("Hand.L", "Hand.R"):
        hand = bpy.data.objects.get(name)
        version = int(hand.get("ps_hand_geometry_version", 0)) if hand is not None else 0
        if hand is None or hand.type != "MESH" or version < REQUIRED_HAND_GEOMETRY_VERSION:
            raise RuntimeError(
                f"'{name}' is missing reusable hand geometry version "
                f"{REQUIRED_HAND_GEOMETRY_VERSION}. Run upgrade_powersuit_model.py."
            )
