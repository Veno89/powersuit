...
from powersuit_pipeline_common import (  # noqa: E402
    ...
    REQUIRED_HAND_GEOMETRY_VERSION,
    require_blender_52,
    save_current_blend,
)
...
def build_hand(builder: MeshBuilder):
    # Closed mechanical glove around a standard grip cavity.  Both weapon grip
    # hardpoints orient the Hand bone so the physical handle occupies the same
    # local cavity (near x=0, y=-0.02, z=0).  Earlier geometry filled that cavity
    # with a solid box, making zero wrist-target error look like severe clipping.
    # The cuff and palm now sit behind the handle while four separated finger pads
    # return around its far side.  This remains a generic closed-grip silhouette;
    # it does not bake sniper-specific geometry into the character.
    builder.add_box((0.0, 0.0, 0.105), (0.120, 0.120, 0.040))
    builder.add_box((0.0, 0.055, 0.015), (0.116, 0.038, 0.126))

    finger_z = (0.054, 0.026, -0.002, -0.030)
    for z in finger_z:
        # Far-side pad and the outer return that visibly connect it to the palm.
        builder.add_box((-0.004, -0.082, z), (0.096, 0.024, 0.021))
        builder.add_box((-0.068, -0.016, z), (0.020, 0.102, 0.021))

    # Opposed thumb wraps the other side of the cavity and crosses toward the
    # upper finger pad.  The small overlaps are intentional glove joints, not
    # disconnected floating blocks.
    builder.add_box((0.066, 0.008, 0.045), (0.022, 0.080, 0.030))
    builder.add_box((0.046, -0.047, 0.045), (0.050, 0.030, 0.028))
...
def rebuild_object(obj: bpy.types.Object, mats) -> None:
    ...
    if obj.name in {"Hand.L", "Hand.R"}:
        obj["ps_hand_geometry_version"] = REQUIRED_HAND_GEOMETRY_VERSION
