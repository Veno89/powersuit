...
class StanceProfile:
    ...
    aiming_eye_reference_semantic: str
    aiming_eye_outward_m: float
    sight_lateral_tolerance_m: float
    sight_vertical_tolerance_m: float
    sight_front_min_m: float
    sight_front_max_m: float
    sight_axis_tolerance_deg: float
    sight_ray_preferred_deg: float
    sight_ray_tolerance_deg: float
...
STANCE_PROFILES = {
    "shouldered_precision": StanceProfile(
        ...
        head_yaw_limit_deg=6.0,
        head_pitch_limit_deg=10.0,
        head_roll_limit_deg=6.0,
        # The visor is much wider than a human face, so a precision stance uses
        # an explicit firing-side eye region rather than incorrectly aiming the
        # ocular at the geometric centre of the entire visor plate.
        aiming_eye_reference_semantic="firing_side_visor_receptor_proxy",
        aiming_eye_outward_m=0.055,
        # A real sighting envelope: the helmet may settle toward the optic, but
        # an ocular outside the firing-side eye region remains a blocker.
        sight_lateral_tolerance_m=0.035,
        sight_vertical_tolerance_m=0.040,
        sight_front_min_m=0.015,
        sight_front_max_m=0.200,
        sight_axis_tolerance_deg=5.0,
        sight_ray_preferred_deg=15.0,
        sight_ray_tolerance_deg=25.0,
    ),
}
...
def tag_contact_surface(obj: bpy.types.Object, role: str) -> None:
    if role not in {ROLE_PRIMARY_GRIP, ROLE_SUPPORT_GRIP, ROLE_STOCK_CONTACT}:
        raise ValueError(f"Unsupported weapon contact-surface role '{role}'.")
    obj["ps_weapon_contact_surface_role"] = role
...
def weapon_contact_surfaces(root: bpy.types.Object, role: str) -> list[bpy.types.Object]:
    return [
        child for child in root.children
        if child.type == "MESH"
        and str(child.get("ps_weapon_contact_surface_role", "")) == role
    ]
...
RIGID_AUTHORED_MATRIX_PROPERTY = "ps_weapon_authored_matrix_v5"
RIGID_MESH_BAKED_PROPERTY = "ps_weapon_mesh_transform_baked_v5"
...
def _canonical_float(value: float, decimals: int) -> float:
    ...
    if not math.isfinite(raw):
        raise RuntimeError(f"Rigid weapon data contains a non-finite value: {raw!r}.")
...
def weapon_local_matrix(
    ...
    Mesh children in rigid-framework v5 have their authored placement baked into
...
def normalize_rigid_weapon_children(root: bpy.types.Object) -> None:
    ...
    harmless visually but made earlier rigidity hashes unstable. Framework v5
...
def _mesh_signature(mesh: bpy.types.Mesh) -> str:
    ...
    for polygon in mesh.polygons:
        digest.update(
            (
                "f=" + ",".join(str(int(index)) for index in polygon.vertices)
                + f"|m={int(polygon.material_index)};"
            ).encode("ascii")
        )
...
def compute_rigid_manifest(root: bpy.types.Object) -> dict[str, object]:
    ...
    for child in sorted(root.children, key=lambda item: item.name):
        ...
        entry: dict[str, object] = {
            ...
            "helper_role": str(child.get("ps_weapon_helper_role", "")),
            "component_role": str(child.get("ps_weapon_component_role", "")),
            "target_semantic": str(child.get("ps_weapon_target_semantic", "")),
            "contact_offset_local": [
                _canonical_float(value, RIGID_TRANSFORM_DECIMALS)
                for value in child.get("ps_weapon_contact_offset_local", ())
            ],
    ...
    return {
        ...
        "root_contract": {
            "contract_version": int(root.get("ps_weapon_contract_version", 0)),
            "weapon_id": str(root.get("ps_weapon_id", "")),
            "weapon_family": str(root.get("ps_weapon_family", "")),
            "stance_family": str(root.get("ps_weapon_stance_family", "")),
            "forward_axis": str(root.get("ps_weapon_forward_axis", "")),
            "up_axis": str(root.get("ps_weapon_up_axis", "")),
        },
        "children": children,
    }
...
def validate_weapon_contract(
    ...
    *,
    ...
    root_values = [
        float(root.matrix_world[row][column])
        for row in range(4)
        for column in range(4)
    ]
    root_values.extend(float(value) for value in root.scale)
    if not all(math.isfinite(value) for value in root_values):
        raise RuntimeError("Weapon root has a non-finite transform.")
    determinant = float(root.matrix_world.to_3x3().determinant())
    ...
    for role in (ROLE_PRIMARY_GRIP, ROLE_SUPPORT_GRIP):
        helper = helpers[role]
        semantic = str(helper.get("ps_weapon_target_semantic", ""))
        if semantic != "wrist_head":
            raise RuntimeError(
                f"Grip helper '{helper.name}' ({role}) must declare wrist_head semantics."
            )
        raw_offset = helper.get("ps_weapon_contact_offset_local")
        if raw_offset is None or len(raw_offset) != 3:
            raise RuntimeError(
                f"Grip helper '{helper.name}' ({role}) has no 3D contact offset."
            )
        offset = Vector(tuple(float(value) for value in raw_offset))
        if not all(math.isfinite(float(value)) for value in offset):
            raise RuntimeError(
                f"Grip helper '{helper.name}' ({role}) has a non-finite contact offset."
            )

    for role in (ROLE_PRIMARY_GRIP, ROLE_SUPPORT_GRIP, ROLE_STOCK_CONTACT):
        surfaces = weapon_contact_surfaces(root, role)
        if len(surfaces) != 1:
            raise RuntimeError(
                f"Weapon contract requires exactly one '{role}' contact surface; "
    ...
    return {
        ...
        "primary_grip_semantic": str(
            helpers[ROLE_PRIMARY_GRIP]["ps_weapon_target_semantic"]
        ),
        "support_grip_semantic": str(
            helpers[ROLE_SUPPORT_GRIP]["ps_weapon_target_semantic"]
        ),
        "primary_grip_contact_offset_local": tuple(
            float(v) for v in helpers[ROLE_PRIMARY_GRIP]["ps_weapon_contact_offset_local"]
        ),
        "support_grip_contact_offset_local": tuple(
            float(v) for v in helpers[ROLE_SUPPORT_GRIP]["ps_weapon_contact_offset_local"]
        ),
