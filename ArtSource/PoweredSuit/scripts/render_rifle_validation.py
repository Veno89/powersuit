...
def _validate_contact_geometry(root: bpy.types.Object) -> dict[str, object]:
    ...
    if int(root.get("ps_generator_version", 0)) < 109:
        raise RuntimeError("Rifle predates the mirrored-shoulder stock correction.")
    ...
    support_helper_object = require_weapon_helper(root, ROLE_SUPPORT_GRIP)
    grip_center = weapon_local_position(root, required["Rifle_SupportGrip"])
    support_vector = support_helper - grip_center
    support_to_grip = support_vector.length
    support_declared = Vector(tuple(
        float(value)
        for value in support_helper_object["ps_weapon_contact_offset_local"]
    ))
    support_vector_error = (support_vector - support_declared).length

    primary_helper = weapon_local_position(
        root, require_weapon_helper(root, ROLE_PRIMARY_GRIP)
    )
    primary_helper_object = require_weapon_helper(root, ROLE_PRIMARY_GRIP)
    pistol_center = weapon_local_position(root, required["Rifle_PistolGrip"])
    primary_vector = primary_helper - pistol_center
    primary_to_grip = primary_vector.length
    primary_declared = Vector(tuple(
        float(value)
        for value in primary_helper_object["ps_weapon_contact_offset_local"]
    ))
    primary_vector_error = (primary_vector - primary_declared).length
    # Grip helpers are wrist transforms, not centres embedded inside solid grip
    # geometry.  Both hardpoints must keep the wrist a plausible palm offset from
    # the physical grip; near-zero is just as wrong as a floating distant target.
    wrist_offset_min = 0.045
    wrist_offset_max = 0.110
    if not wrist_offset_min <= support_to_grip <= wrist_offset_max:
        blockers.append(
            f"support wrist/foregrip offset={support_to_grip:.3f} m"
        )
    if not wrist_offset_min <= primary_to_grip <= wrist_offset_max:
        blockers.append(
            f"primary wrist/pistol-grip offset={primary_to_grip:.3f} m"
        )
    if support_vector_error > 0.005:
        blockers.append(
            f"support wrist offset-vector error={support_vector_error:.3f} m"
        )
    if primary_vector_error > 0.005:
        blockers.append(
            f"primary wrist offset-vector error={primary_vector_error:.3f} m"
        )
    ...
    return {
        ...
        "support_wrist_offset_vector_local": tuple(float(v) for v in support_vector),
        "primary_wrist_offset_vector_local": tuple(float(v) for v in primary_vector),
        "support_wrist_declared_offset_vector_local": tuple(float(v) for v in support_declared),
        "primary_wrist_declared_offset_vector_local": tuple(float(v) for v in primary_declared),
        "support_wrist_offset_vector_error_m": support_vector_error,
        "primary_wrist_offset_vector_error_m": primary_vector_error,
        "grip_helper_semantics": "wrist_transform",
        "grip_wrist_offset_min_m": wrist_offset_min,
        "grip_wrist_offset_max_m": wrist_offset_max,
...
def _render(
    ...
    if view == "left":
        target = center
        location = center - right * (radius * 2.15) + up * (radius * 0.06)
        ...
        target = center
        location = center + forward * (radius * 1.65) - right * (radius * 0.66) + up * (radius * 0.28)
        camera.data.lens = 60.0
    elif view == "rear_3q":
        target = center
        location = center - forward * (radius * 1.65) + right * (radius * 0.66) + up * (radius * 0.28)
        ...
        location = (
            target
            + body_forward * (radius * 1.90)
            + body_right * (radius * 0.76)
            + body_up * (radius * 0.30)
        )
    ...
    render_scene.view_layers[0].update()
    # A required silhouette view is invalid if it crops the very stock or muzzle
    # it is meant to prove.  Verify every framed mesh bound against the actual
    # render camera with a small composition margin before spending render time.
    projected = [
        world_to_camera_view(render_scene, camera, obj.matrix_world @ Vector(corner))
        for obj in framed_meshes
    ...
    if projected:
        ...
        if min_x < 0.025 or max_x > 0.975 or min_y < 0.025 or max_y > 0.975:
            raise RuntimeError(
                f"Rifle validation camera '{view}' crops required geometry: "
