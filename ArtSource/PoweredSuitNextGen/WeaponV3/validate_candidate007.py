"""Fail-closed Blender production gate for Candidate007 / WeaponV3.

This adapter audits the actual visible meshes in the final Candidate007 blend.
...
authorizes Unity integration.  Run through Blender 5.2 or newer, for example:
  ...
  blender --background --python-exit-code 1 --python validate_candidate007.py -- \
    --source ../candidates/nextgen_precision_rifle_candidate_v007.blend \
    --report reports/candidate007_production.json \
    --render-dir ../renders/nextgen_precision_rifle_candidate_v007
"""
...
from weapon_v3_contract import (  # noqa: E402
    ...
    validate_reused_pbr_provenance,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--render-dir",
        type=Path,
        default=NEXTGEN_ROOT / "renders" / "nextgen_precision_rifle_candidate_v007",
    )
    parser.add_argument(
        "--soft-fail",
        action="store_true",
...
def select_lod_objects(
    ...
    if collection is None:
        audit.issue(
            ...
            "error",
            "Every authored WeaponV3 LOD collection is mandatory.",
            actual=None,
    ...
    if unsupported:
        audit.issue(
            ...
            "error",
            "WeaponV3 collections may contain only production mesh renderers.",
            actual=unsupported,
        )
    ...
    for obj in objects:
        ...
        if not obj.name_full.startswith(selection["object_prefix"]):
            audit.issue(
                f"LOD{lod}_OBJECT_ID_INVALID",
                "error",
                "Every Candidate007 renderer must use the NGPR002 object ID prefix.",
                actual=obj.name_full,
                expected=selection["object_prefix"],
            )
        if role not in {"rifle", "optic"}:
            audit.issue(
                f"LOD{lod}_ROLE_INVALID",
                "error",
                "Every visible WeaponV3 mesh requires an explicit rifle/optic role.",
...
def verify_immutable_inputs(
    ...
    for name, expected in sorted(profile["immutable_inputs"].items()):
        try:
            ...
            continue
        hash_mode = str(expected.get("hash_mode", ""))
        try:
            actual_hash = (
                sha256_immutable_input(path, hash_mode) if path.is_file() else None
            )
        except (ContractError, json.JSONDecodeError, UnicodeDecodeError) as exc:
            actual_hash = None
            audit.issue(
                f"IMMUTABLE_{name.upper()}_HASH",
                "error",
                f"Could not hash immutable input with mode {hash_mode!r}: {exc}",
            )
        matches = actual_hash is not None and actual_hash.lower() == expected["sha256"].lower()
        results[name] = {
            "path": report_path(path),
            "hash_mode": hash_mode,
            "required_local_prerequisite": bool(
                expected.get("required_local_prerequisite", False)
            ),
...
def validate_rig_and_actions(
    ...
    if armature is None or armature.type != "ARMATURE":
        audit.issue(
            ...
            "error",
            "Candidate007 must preserve the canonical PowerSuit armature.",
    ...
    audit.issue(
        ...
        "Candidate007 requires WeaponRoot, WeaponMagazine and WeaponBolt to deform their one-hot weighted production renderers.",
    ...
    audit.issue(
        ...
        "Candidate007 must preserve the 30 FPS animation contract.",
...
def validate_weapon_contract(
    ...
    if root is None:
        audit.issue(
            "WEAPON_ROOT_MISSING",
            "error",
            "Candidate007 requires the canonical RifleRoot object.",
    ...
    expected_properties = {
        ...
        "ps_candidate007_hardpoint_version": contract["hardpoint_version"],
    ...
    audit.issue(
        ...
        "RifleRoot metadata must exactly identify the parallel Candidate007 weapon.",
    ...
    audit.issue(
        ...
        "All seven unique direct-child hardpoints must exist with valid canonical geometry.",
...
def load_builder_manifest(source_path: Path) -> tuple[dict[str, Any] | None, str | None]:
    """Load the source-adjacent Candidate007 handoff once, without mutating it."""

    path = source_path.with_suffix(".json")
    try:
        document = json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        return None, None
    return document, sha256_manifest(document)


def validate_candidate007_authoring(
    profile: Mapping[str, Any],
    builder_manifest: Mapping[str, Any] | None,
    audit: Audit,
) -> dict[str, Any]:
    """Require Candidate007's measured 0.33 m stow and symmetric transition proof."""

    start = audit.begin()
    evidence = None
    if isinstance(builder_manifest, Mapping):
        actions = builder_manifest.get("actions")
        if isinstance(actions, Mapping):
            evidence = actions.get("reauthor_evidence")
    errors = validate_stow_evidence(evidence, profile["animation_authoring"])
    if isinstance(builder_manifest, Mapping):
        if builder_manifest.get("candidate") != "nextgen_precision_rifle_candidate_v007":
            errors.append("builder manifest candidate ID differs")
        if builder_manifest.get("asset_id") != profile["weapon"]["weapon_id"]:
            errors.append("builder manifest asset_id differs")
    else:
        errors.append("builder manifest is missing")
    audit.issue(
        "CANDIDATE007_STOW_AUTHORING",
        "pass" if not errors else "error",
        "Candidate007 must bind its measured 0.33 m stow and exact symmetric transition evidence to the source manifest.",
        actual={"evidence": evidence, "errors": errors},
        expected=profile["animation_authoring"],
    )
    audit.report["evidence"]["stow_authoring"] = audit.section_passed(start)
    return {"evidence": evidence, "errors": errors}


def validate_candidate007_manipulation_authoring(
    profile: Mapping[str, Any],
    builder_manifest: Mapping[str, Any] | None,
    audit: Audit,
) -> dict[str, Any]:
    """Require Candidate007's semantic-pad reload/bolt evidence."""
    ...
    errors = validate_manipulation_evidence(
        evidence,
        profile["animation_authoring"]["manipulation"],
        profile["animation_authoring"]["manipulation_densification"],
    )
    if not isinstance(builder_manifest, Mapping):
        errors.append("builder manifest is missing")
    audit.issue(
        "CANDIDATE007_MANIPULATION_AUTHORING",
        "pass" if not errors else "error",
        "Candidate007 must bind measured left/right distal pads, reload seated/detached modes, and the tagged bolt-knob face solve to its source manifest.",
        actual={"evidence": evidence, "errors": errors},
        expected=profile["animation_authoring"]["manipulation"],
    )
    audit.report["evidence"]["manipulation_authoring"] = audit.section_passed(start)
    return {"evidence": evidence, "errors": errors}


def component_role_assignments(
    profile: Mapping[str, Any],
    lod_objects: Mapping[int, Sequence[bpy.types.Object]],
) -> dict[str, list[str]]:
    """Return control-bone assignments grouped by explicit component role."""

    architecture = profile["weapon"]["component_architecture"]
    known_roles = set(architecture["fixed"]["roles"]) | set(
        architecture["articulated"]
    )
    assignments: dict[str, set[str]] = {role: set() for role in known_roles}
    for obj in lod_objects.get(0, []):
        group_names = {group.index: group.name for group in obj.vertex_groups}
        role_attribute = obj.data.attributes.get("weapon_v3_component_role")
        if role_attribute is None or role_attribute.domain != "POINT":
            continue
        role_table_raw = obj.get("weapon_v3_component_role_table_json")
        try:
            role_table = json.loads(str(role_table_raw))
        except Exception:
            continue
        for vertex in obj.data.vertices:
            role = str(role_table.get(str(int(role_attribute.data[vertex.index].value)), ""))
            if role not in known_roles:
                continue
            for influence in vertex.groups:
                if influence.weight > 1.0e-8:
                    control = group_names.get(influence.group)
                    if control:
                        assignments[role].add(control)
    return {role: sorted(controls) for role, controls in sorted(assignments.items())}


def validate_component_architecture(
    profile: Mapping[str, Any],
    lod_objects: Mapping[int, Sequence[bpy.types.Object]],
    builder_manifest: Mapping[str, Any] | None,
    audit: Audit,
) -> dict[str, Any]:
    """Validate fixed structure versus articulated magazine/bolt isolation."""

    start = audit.begin()
    assignments = component_role_assignments(profile, lod_objects)
    manifest_assignments = None
    if isinstance(builder_manifest, Mapping):
        renderers = builder_manifest.get("production_renderers")
        if isinstance(renderers, Mapping):
            architecture = renderers.get("component_architecture")
            if isinstance(architecture, Mapping):
                manifest_assignments = architecture.get("role_control_assignments")
    # Both independent visible-mesh evidence and source-manifest evidence are
    # required; an absent role attribute therefore fails closed.
    visible_errors = validate_component_architecture_evidence(
        {"role_control_assignments": assignments},
        profile["weapon"]["component_architecture"],
    )
    manifest_errors = validate_component_architecture_evidence(
        {"role_control_assignments": manifest_assignments}
        if isinstance(manifest_assignments, Mapping)
        else None,
        profile["weapon"]["component_architecture"],
    )
    errors = [
        *(f"visible: {error}" for error in visible_errors),
        *(f"manifest: {error}" for error in manifest_errors),
    ]
    audit.issue(
        "WEAPON_COMPONENT_ARCHITECTURE",
        "pass" if not errors else "error",
        "Fixed receiver/barrel/stock/handguard/optic-mount vertices must remain on WeaponRoot while magazine and bolt use only their articulated controls.",
        actual={
            "visible_role_control_assignments": assignments,
            "manifest_role_control_assignments": manifest_assignments,
            "errors": errors,
        },
        expected=profile["weapon"]["component_architecture"],
    )
    audit.report["evidence"]["component_architecture"] = audit.section_passed(start)
    return {
        "visible_role_control_assignments": assignments,
        "manifest_role_control_assignments": manifest_assignments,
        "errors": errors,
...
def validate_lods(
    ...
    for lod in range(4):
        ...
        audit.issue(
            ...
            "Each WeaponV3 LOD needs one rifle renderer and at most one optic renderer.",
            actual=dict(roles),
        ...
        audit.issue(
            ...
            "Visible rifle triangles are evaluated against the Candidate007 target.",
        ...
        audit.issue(
            ...
            "Candidate005 suit plus actual Candidate007 visible meshes must meet combined hard ceilings.",
            actual=combined,
            expected={
    ...
    visible_topology_metrics = {
        metric["name"]: metric
        for lod_report in lod_reports
        for metric in lod_report["objects"]
    }
    manifest_topology_metrics = None
    manifest_triangle_counts = None
    if isinstance(builder_manifest, Mapping):
        renderers = builder_manifest.get("production_renderers")
        if isinstance(renderers, Mapping):
            manifest_topology_metrics = renderers.get("topology_metrics")
            manifest_triangle_counts = renderers.get("triangle_counts")
    topology_provenance_errors = validate_topology_provenance(
        visible_topology_metrics,
        manifest_topology_metrics,
        manifest_triangle_counts,
        source_sha256,
        (
            builder_manifest.get("candidate_blend_sha256")
            if isinstance(builder_manifest, Mapping)
            else None
        ),
    )
    audit.issue(
        "WEAPON_TOPOLOGY_PROVENANCE",
        "pass" if not topology_provenance_errors else "error",
        "Every visible WeaponV3 renderer topology must match the source-adjacent builder handoff by exact object identity and counts.",
        actual={
            "visible": visible_topology_metrics,
            "manifest": manifest_topology_metrics,
            "triangle_counts": manifest_triangle_counts,
            "errors": topology_provenance_errors,
        },
    )
    ...
    audit.report["evidence"]["topology_and_uv"] = audit.code_scope_passed(
        start, exact=("WEAPON_TOPOLOGY_PROVENANCE",), suffixes=topology_suffixes
    )
...
def validate_pbr(
    ...
    try:
        ...
        if manifest.get("asset_id") != profile["weapon"]["weapon_id"]:
            errors.append("texture manifest asset_id differs from Candidate007")
    except ContractError as exc:
        manifest = None
        errors = [str(exc)]
    if isinstance(manifest, Mapping):
        try:
            source_input_name = pbr["reuse"]["source_manifest_immutable_input"]
            source_entry = profile["immutable_inputs"][source_input_name]
            source_path = safe_repository_path(
                REPOSITORY_ROOT, source_entry["path"]
            )
            source_manifest = json.loads(source_path.read_text(encoding="utf-8"))
        except Exception as exc:
            errors.append(f"Could not load pinned Candidate006 texture manifest: {exc}")
        else:
            errors.extend(
                validate_reused_pbr_provenance(
                    manifest,
    ...
    audit.issue(
        ...
        "pass" if not errors else "error",
        "Candidate007 requires four existing hash-verified 2K PBR maps.",
    ...
    return {
        ...
        "source_texture_manifest": source_manifest,
...
def validate_clearance_semantics(
    ...
    audit.issue(
        ...
        "WeaponV3 must import the same face-policy identifiers as the clearance gate.",
    ...
    try:
        ...
        if (
            ...
            and manifest.get("contact_window_policy_version")
            != clearance["contact_window_policy_version"]
        ):
            manifest_errors.append(
                "Embedded manifest contact-window policy differs from WeaponV3."
            )
    ...
    for obj in lod0_objects:
        if obj.name_full not in actual_entry_names:
            object_errors.append(
                f"Visible WeaponV3 renderer {obj.name_full} is absent from clearance manifest"
            )
...
def validate_clearance_reports(
    profile: Mapping[str, Any],
    source_path: Path,
    source_sha256: str,
    audit: Audit,
) -> dict[str, Any]:
    """Require all three independently written visible-geometry clearance gates."""

    requirements = profile["clearance"]
    specifications = {
        "authored_clearance": requirements["authored_report_path"],
        "all_frame_clearance": requirements["all_frame_report_path"],
        "dense_transition_clearance": requirements["dense_transition_report_path"],
    }
    results: dict[str, Any] = {}
    source_label = report_path(source_path)
    for kind, raw_path in specifications.items():
        start = audit.begin()
        ...
        try:
            ...
            document = None
            errors = [f"Could not load {kind}: {exc}"]
        else:
            errors = validate_clearance_report(
                ...
                action_ranges=profile["rig"]["action_ranges"],
            )
            try:
                semantic_sha256 = sha256_manifest(document)
            except (TypeError, ValueError) as exc:
                errors.append(
                    f"Could not canonical-hash loaded {kind} document: {exc}"
                )
        audit.issue(
            f"CLEARANCE_{kind.upper()}",
            "pass" if not errors else "error",
            "Candidate007 promotion requires a zero-forbidden visible-geometry clearance report bound to this exact source.",
            actual={"path": report_path(path), "errors": errors},
        )
        audit.report["evidence"][kind] = audit.section_passed(start)
        results[kind] = {
            "path": report_path(path),
...
def run_validation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "profile": {
            ...
            "hash_mode": "canonical_json",
            "sha256": sha256_manifest(profile),
        },
        "source": {
    ...
    audit = Audit(report)
    expected_source = (
        NEXTGEN_ROOT / "candidates" / profile["asset"]["source_filename"]
    ).resolve()
    expected_render_dir = (
        NEXTGEN_ROOT / "renders" / profile["renders"]["directory_name"]
    ).resolve()
    source_id_valid = source_path == expected_source
    render_id_valid = render_dir == expected_render_dir
    report_id_valid = report_output.name == profile["report"]["default_filename"]
    audit.issue(
        "CANDIDATE007_SOURCE_ID",
        "pass" if source_id_valid else "error",
        "The production source path must use the versioned Candidate007 identity.",
        actual=report_path(source_path),
        expected=report_path(expected_source),
    )
    audit.issue(
        "CANDIDATE007_RENDER_ID",
        "pass" if render_id_valid else "error",
        "Review evidence must use the versioned Candidate007 render directory.",
        actual=report_path(render_dir),
        expected=report_path(expected_render_dir),
    )
    audit.issue(
        "CANDIDATE007_REPORT_ID",
        "pass" if report_id_valid else "error",
        "The production report filename must use the Candidate007 report ID.",
        actual=report_output.name,
        expected=profile["report"]["default_filename"],
    )
    if tuple(bpy.app.version[:2]) < (5, 2):
        audit.issue(
            "BLENDER_VERSION",
            "error",
            "Candidate007 production validation requires Blender 5.2 or newer.",
    ...
    builder_manifest, builder_manifest_hash = load_builder_manifest(source_path)
    report["builder_manifest"] = {
        "path": report_path(source_path.with_suffix(".json")),
        "canonical_sha256": builder_manifest_hash,
        "loaded": builder_manifest is not None,
    }
    ...
    lod_objects, report["lods"] = validate_lods(
        profile, armature, builder_manifest, source_hash_before, audit
    )
    report["stow_authoring"] = validate_candidate007_authoring(
        profile, builder_manifest, audit
    )
    report["manipulation_authoring"] = validate_candidate007_manipulation_authoring(
        profile, builder_manifest, audit
    )
    report["component_architecture"] = validate_component_architecture(
        profile, lod_objects, builder_manifest, audit
    )
    ...
    report["clearance_reports"] = validate_clearance_reports(
        profile, source_path, source_hash_before, audit
    )
    ...
    audit.issue(
        ...
        "Validation must not mutate the Candidate007 source blend.",
...
def main() -> int:
    ...
    print(
        f"WeaponV3 Candidate007 validation: {report['status']} "
