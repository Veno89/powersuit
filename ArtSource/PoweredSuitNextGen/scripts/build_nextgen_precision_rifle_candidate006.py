"""Build Candidate006 / NextGen Precision Rifle 001 in an isolated review blend.
...
OUTPUT_BLEND = NEXTGEN / "candidates" / "nextgen_precision_rifle_candidate_v006.blend"
OUTPUT_REPORT = NEXTGEN / "candidates" / "nextgen_precision_rifle_candidate_v006.json"
RENDER_ROOT = NEXTGEN / "renders" / "nextgen_precision_rifle_candidate_v006"
TEXTURE_ROOT = NEXTGEN / "textures" / "candidate006"
TEXTURE_MANIFEST = TEXTURE_ROOT / "manifest.json"
REAUTHOR_SCRIPT = Path(__file__).resolve().parent / "reauthor_candidate006_weapon_actions.py"
...
RIFLE_COMPONENTS = "NGPR001_SourceComponents"
OPTIC_COMPONENTS = "NGPR001_OpticSourceComponents"
...
def remove_existing_rifle() -> None:
    for obj in list(bpy.data.objects):
        if obj.name == "RifleRoot" or obj.name.startswith("Rifle_") or obj.name.startswith("NGPR001_"):
            data = obj.data
            bpy.data.objects.remove(obj, do_unlink=True)
            if data is not None and getattr(data, "users", 1) == 0 and isinstance(data, bpy.types.Mesh):
                bpy.data.meshes.remove(data)
    for collection in list(bpy.data.collections):
        if collection.name.startswith("WeaponV2_LOD") or collection.name in {RIFLE_COMPONENTS, OPTIC_COMPONENTS}:
...
def image(name: str, *, non_color: bool = False) -> bpy.types.Image:
    ...
    if not path.is_file():
        raise RuntimeError(f"Candidate006 texture is missing: {path}")
...
def build_components(root: bpy.types.Object, components: bpy.types.Collection,
    ...
    add(tapered_box("NGPR_Lower_Keel", (0.0, 0.070, 0.051), (0.086, 0.275, 0.044), components, mat["gunmetal"], front=(0.84, 0.72), bevel=0.004))
    ...
    add(box("NGPR_SupportYoke_Mount", (0.070, 0.360, 0.072), (0.060, 0.070, 0.030), components, mat["chrome"], bevel=0.004), role="support_grip")
    add(tube_between("NGPR_SupportYoke_Upper", Vector((0.084, 0.345, 0.068)), Vector((0.112, 0.315, -0.005)), 0.012, components, mat["gunmetal"], vertices=16), role="support_grip")
    add(tube_between("NGPR_SupportYoke_Lower", Vector((0.112, 0.315, -0.005)), Vector((0.125, 0.285, -0.072)), 0.013, components, mat["gunmetal"], vertices=16), role="support_grip")
    ...
    add(cylinder("NGPR_SupportYoke_HingeCap", (0.080, 0.360, 0.072), 0.018, 0.020, components, mat["gunmetal"], axis="X", vertices=28, bevel=0.0015), role="support_grip")
    add(tapered_box("NGPR_SupportYoke_Guard", (0.123, 0.302, -0.020), (0.010, 0.100, 0.096), components, mat["armor"], front=(0.82, 0.78), bevel=0.002), role="support_grip")
    ...
    add(tapered_box("NGPR_PistolGrip", (-0.070, -0.050, -0.063), (0.050, 0.082, 0.158), components, mat["rubber"], front=(0.78, 0.86), bevel=0.006, rotation=(math.radians(-22), 0.0, 0.0)), face_policy.WEAPON_PRIMARY_GRIP, "primary_grip")
    add(box("NGPR_TriggerGuard", (-0.035, 0.055, -0.002), (0.064, 0.096, 0.022), components, mat["gunmetal"], bevel=0.004), role="primary_grip")
    add(tapered_box("NGPR_GripBackstrap", (-0.070, -0.085, -0.060), (0.056, 0.020, 0.135), components, mat["carbon"], front=(0.86, 0.92), bevel=0.003, rotation=(math.radians(-22), 0.0, 0.0)), face_policy.WEAPON_PRIMARY_GRIP, "primary_grip")
    for index, z in enumerate((-0.020, -0.060, -0.100)):
        add(box(f"NGPR_GripRib_{index}", (-0.070, -0.012, z), (0.054, 0.012, 0.010), components, mat["carbon"], bevel=0.002), face_policy.WEAPON_PRIMARY_GRIP, "primary_grip")
    magazine = add(tapered_box("NGPR_Magazine", (0.0, 0.135, -0.082), (0.072, 0.074, 0.174), components, mat["carbon"], front=(0.76, 0.86), bevel=0.005, rotation=(math.radians(-7), 0.0, 0.0)), face_policy.WEAPON_MAGAZINE_GRASP, COMPONENT_MAGAZINE)
    magazine_base = add(box("NGPR_MagazineBase", (0.0, 0.146, -0.174), (0.080, 0.074, 0.020), components, mat["chrome"], bevel=0.003), face_policy.WEAPON_MAGAZINE_GRASP, COMPONENT_MAGAZINE)
    magazine_collar = add(box("NGPR_MagwellCollar", (0.0, 0.125, 0.010), (0.086, 0.092, 0.022), components, mat["gunmetal"], bevel=0.003), face_policy.WEAPON_MAGAZINE_GRASP, COMPONENT_MAGAZINE)
    magazine_ribs = []
    for side in (-1.0, 1.0):
        magazine_ribs.append(add(box(f"NGPR_MagazineRib_{'R' if side < 0 else 'L'}", (side * 0.034, 0.138, -0.085), (0.008, 0.078, 0.128), components, mat["armor"], bevel=0.002), face_policy.WEAPON_MAGAZINE_GRASP, COMPONENT_MAGAZINE))
    ...
    add(tapered_box("NGPR_Buttpad", (-0.112, -0.442, 0.132), (0.094, 0.026, 0.150), components, mat["rubber"], front=(0.86, 0.90), bevel=0.006), face_policy.WEAPON_BUTTPAD, "stock")
    add(tapered_box("NGPR_ButtHeel", (-0.112, -0.454, 0.070), (0.100, 0.014, 0.035), components, mat["carbon"], front=(0.86, 0.90), bevel=0.003), face_policy.WEAPON_BUTTPAD, "stock")
    ...
    # Articulated bolt geometry stays separate and follows WeaponBolt.
    charging = add(box("NGPR_ChargingRail", (-0.072, 0.050, 0.190), (0.016, 0.160, 0.018), components, mat["chrome"], bevel=0.002), face_policy.WEAPON_BOLT_HANDLE, COMPONENT_BOLT)
    bolt_stem = add(cylinder("NGPR_BoltStem", (-0.099, 0.020, 0.188), 0.007, 0.050, components, mat["chrome"], axis="X", vertices=16), face_policy.WEAPON_BOLT_HANDLE, COMPONENT_BOLT)
    ...
    bolt_knob = add(tapered_box("NGPR_BoltKnob", (-0.130, 0.020, 0.188), (0.020, 0.034, 0.026), components, mat["rubber"], front=(0.82, 0.82), bevel=0.004), face_policy.WEAPON_BOLT_HANDLE, COMPONENT_BOLT)
    ...
    helpers = [
        create_helper("Rifle_PrimaryGrip", ROLE_PRIMARY_GRIP, (-0.070, -0.050, -0.040), components,
                      rotation=(-math.pi / 2, 0.20, -math.pi / 2)),
        create_helper("Rifle_SupportGripTarget", ROLE_SUPPORT_GRIP, (0.108, 0.300, -0.035), components,
                      rotation=(-math.pi / 2, 0.20, -math.pi / 2)),
        ...
        create_helper("Rifle_SupportGripMin", ROLE_SUPPORT_MIN, (0.085, 0.270, -0.035), components),
        create_helper("Rifle_SupportGripMax", ROLE_SUPPORT_MAX, (0.125, 0.335, -0.035), components),
    ]
    for child in [*parts, *optics, *helpers]:
        child.parent = root
        child.parent_type = "OBJECT"
        child.matrix_parent_inverse = Matrix.Identity(4)
    for component in (magazine, magazine_base, magazine_collar, *magazine_ribs):
        tag_component(component, COMPONENT_MAGAZINE)
        component[WEAPON_OWNER_PROPERTY] = ASSET_ID
    for component in (charging, bolt_stem, bolt_collar, bolt_knob):
...
def join_renderer(name: str, objects: list[bpy.types.Object], collection: bpy.types.Collection,
    ...
    result["weapon_v2_role"] = role
    result["weapon_v2_lod"] = lod
...
def copy_renderer(source: bpy.types.Object, name: str, collection: bpy.types.Collection,
    ...
    collection.objects.link(obj)
    obj["weapon_v2_role"] = role
    obj["weapon_v2_lod"] = lod
...
def validate_weapon_skin_contract(
    ...
    for name in controls:
        ...
        if rest_delta > 1.0e-6:
            raise RuntimeError(
                "Candidate006 weapon skin controls no longer share one bind "
                f"matrix: {name} delta={rest_delta:.9f}"
            )
        if not bone.use_deform:
            raise RuntimeError(f"{name} must deform Candidate006 weapon renderers")
        rest_evidence[name] = {
            "use_deform": True,
    ...
    for obj in renderers:
        ...
        if str(obj.get("weapon_v2_role", "")) == "rifle" and any(
...
def unwrap_uv0(objects: list[bpy.types.Object]) -> None:
    for obj in objects:
        ...
        if int(obj.get("weapon_v2_lod", 0)) >= 2:
...
def tag_suit_semantics(armature: bpy.types.Object) -> list[bpy.types.Object]:
    ...
    if len(suit) != 3:
        raise RuntimeError("Candidate006 requires Candidate005's three visible suit renderers")
    ...
    for object_name in ("H2_Armor_LOD0", "H2_Undersuit_LOD0"):
        ...
        for polygon in obj.data.polygons:
            ...
            for side in ("R", "L"):
                ...
                if bone is None:
                    raise RuntimeError(f"Candidate006 requires Hand.{side} rest bone")
    ...
    print(f"CANDIDATE006_HAND_SEMANTICS={payload}")
    if missing:
        raise RuntimeError(
            f"Candidate006 hand tagging failed closed; missing semantic IDs {missing}"
        )
    return suit
...
def add_clearance_manifest(suit: list[bpy.types.Object], weapon: list[bpy.types.Object]) -> dict[str, object]:
    ...
    ready_windows = [
        ...
        for action in sorted(face_policy.READY_ACTIONS)
    ]
...
def create_studio(
    ...
    collection = ensure_collection("NGPR001_ReviewStudio")
...
def validate_weapon_skin_motion(
    ...
    try:
        ...
        if max(transition_errors.values()) > 1.0e-4:
            raise RuntimeError(
                "Candidate006 draw/sheathe endpoints are not reversible: "
        ...
        if min(midpoint_separation.values()) <= 0.01:
            raise RuntimeError(
                "Candidate006 draw/sheathe motion collapsed to a static pose"
            )
        ...
        if articulation["magazine_travel_m"] < 0.25:
            raise RuntimeError("Candidate006 magazine travel is below 0.25 m")
        if articulation["bolt_travel_m"] < 0.08:
            raise RuntimeError("Candidate006 bolt travel is below 0.08 m")
        if max(
            articulation["magazine_return_error"],
            articulation["bolt_return_error"],
        ) > 1.0e-4:
            raise RuntimeError("Candidate006 articulation does not return to rest")
        return {
            "schema_version": 1,
...
def render_reviews(
    ...
    for obj in bpy.data.objects:
        if obj.get("weapon_v2_lod") is not None and int(obj["weapon_v2_lod"]) != 0:
    ...
    jobs = [
        ...
        (RENDER_NAMES[12], "PS_Weapon_Sheathe", 21, Vector((-2.40, -4.10, 1.45)), Vector((0.0, -0.02, 1.16)), 72.0),
    ]
...
def main() -> None:
    if bpy.app.version < (5, 2, 0):
        raise RuntimeError("Candidate006 requires Blender 5.2 or newer")
    ...
    root["ps_support_grip_x_local"] = 0.108
    root["ps_scope_x_local"] = 0.0
    root["ps_candidate006_hardpoint_version"] = "NGPR001_HARDPOINTS_V1"
    ...
    lod_collections = {lod: ensure_collection(f"WeaponV2_LOD{lod}") for lod in range(4)}
    ...
    rifle_lod0 = join_renderer("NGPR001_Rifle_LOD0", render_sources, lod_collections[0], mat["armor"], "rifle", 0, armature)
    optic_lod0 = join_renderer("NGPR001_Optic_LOD0", optic_sources, lod_collections[0], mat["glass"], "optic", 0, armature)
    ...
    for lod in (1, 2, 3):
        rifle = copy_renderer(rifle_lod0, f"NGPR001_Rifle_LOD{lod}", lod_collections[lod], LOD_TARGETS[lod], "rifle", lod)
        optic = copy_renderer(optic_lod0, f"NGPR001_Optic_LOD{lod}", lod_collections[lod], max(64, len(optic_lod0.data.polygons) // (lod + 1)), "optic", lod)
    ...
    # Semantic zones on consolidated LOD0 are reconstructed conservatively by
    # proximity to immutable hardpoint envelopes; lower LODs intentionally fail
    # closed as ordinary geometry.
    semantic_centers = [
        (Vector((-0.070, -0.050, -0.063)), 0.14, face_policy.WEAPON_PRIMARY_GRIP),
        (Vector((0.108, 0.300, -0.045)), 0.14, face_policy.WEAPON_SUPPORT_GRIP),
        (Vector((-0.112, -0.430, 0.132)), 0.12, face_policy.WEAPON_BUTTPAD),
        (Vector((0.0, 0.135, -0.082)), 0.13, face_policy.WEAPON_MAGAZINE_GRASP),
        (Vector((-0.105, 0.025, 0.188)), 0.10, face_policy.WEAPON_BOLT_HANDLE),
    ]
    lod0_values = []
    for polygon in rifle_lod0.data.polygons:
        value = face_policy.WEAPON_ORDINARY
        for center, radius, zone in semantic_centers:
            if (polygon.center - center).length <= radius:
                value = zone
                break
        lod0_values.append(value)
    create_face_attribute(rifle_lod0, face_policy.WEAPON_ATTRIBUTE, face_policy.WEAPON_ORDINARY, lod0_values)
    create_face_attribute(optic_lod0, face_policy.WEAPON_ATTRIBUTE, face_policy.WEAPON_ORDINARY)
    ...
    root["weapon_v2_texture_manifest_json"] = json.dumps(
        {
            "schema_version": 1,
    ...
    if not REAUTHOR_SCRIPT.is_file():
        raise RuntimeError(f"Candidate006 action reauthor stage is missing: {REAUTHOR_SCRIPT}")
    reauthor_module = load_module("candidate006_action_reauthor", REAUTHOR_SCRIPT)
    reauthor_evidence = reauthor_module.reauthor_candidate006_weapon_actions(armature, root)
    ...
    if not isinstance(carrier_values, list) or len(carrier_values) != 16:
        raise RuntimeError("Candidate006 reauthor did not return a 4x4 carrier invariant")
    ...
    if after != before:
        raise RuntimeError("Immutable Candidate005 source changed during Candidate006 generation")
    ...
    if sha256(OUTPUT_BLEND) != final_blend_hash:
        raise RuntimeError("Final Candidate006 blend changed after render evidence binding")
    ...
    report = {
        "schema_version": 1,
        "candidate": "Candidate006 / NextGen Precision Rifle 001",
        "status": "ISOLATED_PRODUCTION_WEAPON_CANDIDATE_NOT_UNITY_INTEGRATED",
        "asset_id": ASSET_ID,
        "source_candidate005": str(SOURCE_BLEND),
        ...
        "hardpoint_contract": {
            "version": root["ps_candidate006_hardpoint_version"],
            ...
            "forward_axis": "+Y",
            "up_axis": "+Z",
            "support_grip_dogleg_local_m": [0.108, 0.300, -0.035],
            "primary_grip_local_m": [-0.070, -0.050, -0.040],
        ...
        "production_renderers": {
            ...
            "per_lod": {f"LOD{lod}": [obj.name for obj in lod_renderers if int(obj["weapon_v2_lod"]) == lod] for lod in range(4)},
        ...
        "limitations": [
            ...
            "Candidate006 weapon actions were solver-reauthored around the new immutable hardpoints; visible strict clearance remains the promotion gate.",
