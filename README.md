...
- `PowerSuitCombatSandbox.prefab` supplies three connected greybox zones, separate ground/flight spawn regions, 19 spawn points, a foundry catwalk, causeway bridge/AoE courtyard, and airfield hover platforms/flight gates. Ground candidates are projected onto the surface and must pass full capsule-clearance checks, so enemies cannot be accepted inside the sandbox blocks. `PowerSuitEncounterDirector` advances a fixed causeway (7), foundry (7), and airfield (9) enemy sequence as the player reaches each area, restarts the active phase after defeat, and leaves the general deterministic SpawnDirector available for console/stress use.
- Suit sprint/flight exhaust uses emissive line-renderer jets without moving real-time point lights, avoiding animated floor-light flicker while retaining weapon and ability impact lighting.
...
- Windows x64 Development Build completed successfully on 2026-08-11 after the spawn-clearance and floor-lighting hotfix. Only non-blocking package-level Sentis shader warnings were reported.
