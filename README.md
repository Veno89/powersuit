...
- Ground locomotion has acceleration/deceleration, signed directional movement, coyote time, jump buffering, air control, landing response, and backpedalling.
- Flight includes takeoff, hover, ascend/descend, braking, boost, banking, landing transitions, and flight-compatible aiming, firing, reloading, and abilities.
- Camera profiles cover exploration, shoulder aim, flight, boost, ability targeting, and the Precision Rifle's actual `ScopePoint`. Collision pull-in and damped release avoid recurrent camera clipping.
...
- The HUD presents player health, ammo/reload, reticle/hit state, ability cooldowns, and ultimate meter. Enemy health bars are camera-facing mesh renderers with distance culling, avoiding per-enemy Canvas rebuilds; encounter counts remain available through console statistics.
